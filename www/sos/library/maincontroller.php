<?	
	...
	{		
		...
		{ 
			...
			//db setting
			$this->sysparam['db']['server'] 		= 'TAPEBACKUP\BKUPEXEC';
			$this->sysparam['db']['name'] 			= 'WEB';
			$this->sysparam['db']['user'] 			= 'sa';
			$this->sysparam['db']['password'] 		= 'sa';	
			
			//database to send short message service
			$this->sysparam['dbsms']['server'] 		= 'NSUDBS';
			$this->sysparam['dbsms']['name'] 		= 'WEB';
			$this->sysparam['dbsms']['user'] 		= 'sos';
			$this->sysparam['dbsms']['password'] 	= 'S0s#0k';	
			
			//application parameter			
			$this->sysparam['app']['bcurl']				= "http://webdev.sophiemartin.com/sos/index.php";
			$this->sysparam['app']['mbrurl']			= "http://webdev.sophiemartin.com/sos/mbraccess.php";
			...
			$this->sysparam['salesstatus']['delivered']		= 9; //bc yang update kalo barang sudah di delivered. 
			$this->sysparam['salesstatus']['clear']			= 10; //bc yang update kalo barang sudah di delivered. 
			...
			$this->sysparam['purchstatus']['clear']			= 10;
		...
		function gotopage($page) { /* inherited */ }	

		function updatesalesstatus($salesid, $status)
		{
			$date = getdate();
			...
			switch($status)
			{
				...
				case $this->sysparam['salesstatus']['validated']:
					$sql = "exec sp_updateSalesStatus " . $this->queryvalue($salesid) . "," . $status;
					echo $sql;
					$this->db->execute($sql);
					break;
			}
